//1-9-15 KB added my changes to handle desired behavior for the mouse wheel movements
...
public class MouseEventManagerOJ implements MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    public void replaceMouseListener(ImagePlus imp) {
    	//KB 1-9-15 made imageJ move in Z by default when the wheel moves
        if (imp != null) {
            ...
            imp.getCanvas().removeMouseWheelListener(MouseEventManagerOJ.getInstance());
            
            imp.getCanvas().removeMouseListener(imp.getCanvas());
            imp.getCanvas().removeMouseMotionListener(imp.getCanvas());
         
            
        	MouseWheelListener[] MouseWheelListenerArray = imp.getWindow().getMouseWheelListeners();
			for(int i =0; i<MouseWheelListenerArray.length; i++)
        		imp.getWindow().removeMouseWheelListener(MouseWheelListenerArray[i]);
            ...
            imp.getWindow().addMouseWheelListener(MouseEventManagerOJ.getInstance());
        ...
        if (imp2 != null) {
            ...
            if (OJ.isValidData() && impLinked(imp2) && (OJ.getToolStateProcessor().getToolState() != ToolStateProcessorOJ.STATE_NONE) && (!IJ.spaceBarDown())) {//4.7.2009
                if ((imp2 instanceof CompositeImage) && (((CompositeImage) imp2).getMode() == CompositeImage.COMPOSITE)) {
                    ...
                    int prefCheck=(int) Prefs.get("marker.channelMustBeActive", 1);
                    if (prefCheck==1){
    	...
    	if (KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusedWindow() instanceof ImageWindow) {
    		...
    		synchronized(this) {
    			...
    			if (imp.isHyperStack()) {
    				...
    				SyncWindows.setZ(imp.getWindow(), slice);
    			}
