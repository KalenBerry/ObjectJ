/**
 * Qualifiers Panel within Project window
 * fully documented 11.2.2010
 */
 ...
 * - QualifiersHeaderRenderer
 * - QualifiersTableModel
 * - QualifiersTableRenderer
 */
public class QualifiersSettingsOJ extends javax.swing.JPanel implements TableColumnModelListener, IControlPanelOJ, CellChangedListenerOJ, ColumnChangedListenerOJ, QualifierChangedListenerOJ {
  ...
  private void fillColumnSelector() {
    ...
    if (OJ.isValidData()) {
      for (int i = 0; i < OJ.getData().getResults().getColumns().getAllColumnsCount(); i++) {
        columnSelector.addItem(OJ.getData().getResults().getColumns().getColumnByIndex(i).getName());
      }
    }
  ...
  /**
   * puts all available conditions (such as >, ==,  etc) into  names into a combo box
   */
  ...
  /**
   * Listener called when column is changed, or when a new conditions is added to the bottom
   */
  ...
  /**
   * For leaving the editing mode after pressing one of the radiobuttons "All", "If" etc
   */
  ...
  /**
   * updates radio buttons, counts qualified flags of all objects and updates the hit results
   */
  ...
  public class QualifiersHeaderRenderer extends JLabel implements TableCellRenderer {
    ...
    /**
     * return table header cell in form of correctly formated label (always black)
     */
  ...
  public class QualifiersTableRenderer extends javax.swing.JLabel/*RoundLabelOJ*/ implements TableCellRenderer {
    ...
    /**
     * return table cell in form of correctly formated label (gray if conditions overruled by "All" etc)
     */
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
...
//                    return QualifierOJ.getOperationText(qualifier.getOperation());
    ...
    /**
     * e.g. a cell is not editable if it is below the last row of qualifiers, or if condition does not require a numeric value
     */
    ...
    /**
     * accepts a qualifier object, that corresponds to one row in the Qualifier settings
     * @param qualifier
     * @return
     */
