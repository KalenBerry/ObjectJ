 ...
 * - choosing visibility of columns (a list, bottom left)
 * - showing results (a 2D table, right)
 */
public class ProjectResultsOJ extends javax.swing.JFrame implements TableColumnModelListener, CellChangedListenerOJ, YtemChangedListenerOJ,
  ...
  /**
   * May be called fom one of the four tables:
   * - Linked Statistics (=header)
   * - Linked values
   * - Unlinked Statistics (=header)
   * - Unlinked values
   * @param event
   */
...
//+++exception at bindingGroup.bind(); 4 lines above
      ...
      if (evt.getButton() == java.awt.event.MouseEvent.BUTTON1) {
        ...
        if (index >= 0) {
          ...
          try {
            ...
            if (evt.getClickCount() == 2) { // TODO looks like a double click! (ET)
  ...
  /**
   *column names must be unique, not contain * and ?, and start
   *with a letter. (We want to reserve other begin characters for system)
   *Column names are not case sensitive.
   *@param name column title to be tested
   *@return -1 if conditions are met; -2 wrong characters; >=0 conflicting column index.
   */
