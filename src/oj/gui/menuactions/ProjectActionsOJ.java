...
import oj.macros.MacroProcessorOJ;//KB added could make things screwy
...
public class ProjectActionsOJ {
    ...
    private static void openImagesFromLastProject(){
    	...
    	for (int i=0;i<strArray.length;i++){
    			String iName=OJ.getData().getImages().getImageByIndex(Integer.parseInt(strArray[i])-1).getFilename();
    ...
    private static boolean closeProjectData() {
        ...
        for(int iNum=1;iNum<=numImages;iNum++){
        	boolean openCheck=OJ.getData().getImages().getImageByIndex(iNum-1).getID()<0;
        	if(openCheck){
