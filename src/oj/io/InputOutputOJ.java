...
public boolean saveProjectAs(DataOJ data, boolean itsBinary, boolean pathMayChange) {
     ...
     */
    //public boolean saveProjectAs(DataOJ data, boolean itsBinary, boolean saveACopy/*, boolean withContent*/) {
    public boolean f(DataOJ data, boolean itsBinary, boolean pathMayChange) {
    ...
    private boolean saveProject(DataOJ data, String directory, String filename, boolean asBinary) {
        ...
        try {
        	...
        	//save some info about the projects for reopening the last closed project later;
   
        	String imagesOpenLastProject="";
        	boolean intBool=false;
        	int numImages=data.getImages().getImagesCount();
        for(int iNum=0;iNum<numImages;iNum++){
        	boolean openCheck=data.getImages().getImageByIndex(iNum).getID()<0;
        	if (openCheck){
        		intBool=true;
        	...
        	if (intBool){
        		imagesOpenLastProject=imagesOpenLastProject.substring(0,imagesOpenLastProject.length()-1);
        	}
