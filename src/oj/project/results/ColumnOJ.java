...
import java.util.HashMap; // needed to do fancy stuff to count objects relative to image
...
//   }
    ...
    private void updateValues() {
        ...
        HashMap<String, Integer> relObjNumMap = new HashMap<String, Integer>();
        ...
        for (int i = 0; i < ((DataOJ) parent.getParent().getParent()).getCells().getCellsCount(); i++) {
            ...
            // TODO : code inserted here to allow easy reporting of obj. number relative to image
            // implements a map with keys as images or image names, and values as the number of points encountered so far
            if (!(relObjNumMap.containsKey(im.getFilename()))) // insert key when it doesn't exist
            {
            	relObjNumMap.put(im.getFilename(), 1);
            }
            else
            {
            	int currentNumber = relObjNumMap.get(im.getFilename());
            	currentNumber++;
            	relObjNumMap.put(im.getFilename(), currentNumber); // increment by one?
            }
            ...
            switch (columnDef.getAlgorithm()) {
                ...
                case ColumnDefOJ.ALGORITHM_CALC_OBJECT_NUMBER_RELATIVE_TO_IMAGE:
                	// TODO : here's where we can finally report the number of this object 
                	// relative to all the objects assigned to this image!
                	int numToSet = relObjNumMap.get(im.getFilename());
                	setDoubleResult(i, numToSet);
                	break;
                	
                case ColumnDefOJ.ALGORITHM_CALC_PIXEL_AREA_SUM://KB 1/13/15
                	setDoubleResult(i, Double.NaN);
                	break;
                	
                	/*I realized that the errors from the below code came about because I assumed the images would be open. That is wrong. Need to do this a different way with a macro or plugin. The marker type I added is still useful here. 


                	ytm3 = cell.getClone(columnDef.getOperand(0).getYtemClone(), columnDef.getOperand(0).getObjectName());
                 	int type = im.getImagePlus().getType();
                 	boolean typeTest=ImagePlus.GRAY8==type | ImagePlus.GRAY16==type |ImagePlus.GRAY32==type;
                 	int area_sum=0;
                	if (ytm3 != null & typeTest  ) {
                		
                		int channelOfChoice=1;
                		int squareRadius=2;

                		int xPos=(int) Math.round(getScaledX(i, ytm3.getLocation(columnDef.getOperand(0).getRelPosition()).getX()));
                		int yPos=(int) Math.round(getScaledY(i, ytm3.getLocation(columnDef.getOperand(0).getRelPosition()).getY()));

                		int squareXCorner= xPos-squareRadius;
                		int squareYCorner= yPos-squareRadius;

                		int index=(int) Math.round(getScaledZ(i, ytm3.getLocation(columnDef.getOperand(0).getRelPosition()).getZ()));
                		int numChannels=im.getNumberOfChannels();

                		int slice= (int) Math.ceil(index/numChannels); // converts index into slice position

                		im.getImagePlus().setPositionWithoutUpdate(channelOfChoice,slice,1);

                		for(int xIndex=squareXCorner;xIndex<=xPos+squareRadius;xIndex++){

                			for(int yIndex=squareYCorner;yIndex<=yPos+squareRadius;yIndex++){
                				int[] pixel=im.getImagePlus().getPixel(xIndex, yIndex);
                				area_sum+=pixel[0];//first pixel of get pixel command above is gray value. Only gray types should get to this point 
                				System.out.println("point : "+i+" Pixel X: "+xIndex+" Pixel Y: "+yIndex+" has a value of : "+pixel[0]+ " the current sum is "+area_sum);
                			}
                		}
                		setDoubleResult(i, area_sum);
                	} else {
                		setDoubleResult(i, Double.NaN);
                	}
                	break;
