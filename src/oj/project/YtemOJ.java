...
public abstract class YtemOJ extends BaseAdapterOJ {
    ...
    /** @return true if any location was changed*/
    public boolean getChanged() {
        if (super.getChanged()) {
            return true;
        } else {
            for (int i = 0; i < locations.size(); i++) {
                if (((LocationOJ) locations.get(i)).getChanged()) {
                    return true;
                }
            }
        }
    ...
    /** propagates flag to all locations (why?)*/
    public void setChanged(boolean changed) {
        super.setChanged(changed);
        for (int i = 0; i < locations.size(); i++) {
            ((LocationOJ) locations.get(i)).setChanged(changed);
        }
    }
    ...
    public LocationOJ getLocation(int index) {
        if (UtilsOJ.inRange(0, locations.size() - 1, index)) {
            return (LocationOJ) locations.get(index);
        } else {
    ...
    public LocationOJ setLocation(int index, LocationOJ newLocation) {
        if (UtilsOJ.inRange(0, locations.size() - 1, index)) {
            LocationOJ old_location = (LocationOJ) locations.get(index);
    ...
    public double[] toYDArray() {
        ...
        double[] result = new double[locations.size()];
        for (i = 0; i < locations.size(); i++) {
            result[i] = ((LocationOJ) locations.get(i)).y;
        }
        return result;
    }
    ...
    public int[] toXArray() {
        ...
        for (i = 0; i < result.length; i++) {
            double val = ((LocationOJ) locations.get(i)).x;
            result[i] = (int) Math.round(val);//20.3.2009
            //result[i] = (int) ((LocationOJ) locations.get(i)).x;
        }
        return result;
    }
    ...
    public int[] toYArray() {
        ...
        int[] result = new int[locations.size()];
        for (i = 0; i < result.length; i++) {
            double val = ((LocationOJ) locations.get(i)).y;
            result[i] = (int) Math.round(val);//20.3.2009
            //result[i] = (int) ((LocationOJ) locations.get(i)).y;
        }
        return result;
    }
    ...
    public int[] toZArray() {
        ...
        int[] result = new int[locations.size()];
        for (i = 0; i < result.length; i++) {
            double val = ((LocationOJ) locations.get(i)).z;
            result[i] = (int) Math.round(val);//20.3.2009
            //result[i] = (int) ((LocationOJ) locations.get(i)).z;
        }
        return result;
    }
    ...
    public int[] toXArray(int offset, double magnification) {
        ...
        int[] result = new int[locations.size()];
        for (i = 0; i < result.length; i++) {
            //result[i] = (int) ((((LocationOJ) locations.get(i)).x - offset) * magnification);
            double val = ((((LocationOJ) locations.get(i)).x - offset) * magnification);
            result[i] = (int) Math.round(val);//20.3.2009
        }
        return result;
    }
    ...
    public int[] toYArray(int offset, double magnification) {
        ...
        int[] result = new int[locations.size()];
        for (i = 0; i < result.length; i++) {
            //result[i] = (int) ((((LocationOJ) locations.get(i)).y - offset) * magnification);
            double val = ((((LocationOJ) locations.get(i)).y - offset) * magnification);
            result[i] = (int) Math.round(val);//20.3.2009
        }
        return result;
    }
    ...
    public Rectangle getRectangle() {
        if (locations.size() > 0) {
            double x1 = ((LocationOJ) locations.get(0)).getX();
            double y1 = ((LocationOJ) locations.get(0)).getY();
            double x2 = ((LocationOJ) locations.get(0)).getX();
            double y2 = ((LocationOJ) locations.get(0)).getY();
            for (int i = 0; i < locations.size(); i++) {
                x1 = Math.min(x1, ((LocationOJ) locations.get(i)).getX());
                y1 = Math.min(y1, ((LocationOJ) locations.get(i)).getY());
                x2 = Math.max(x2, ((LocationOJ) locations.get(i)).getX());
                y2 = Math.max(y2, ((LocationOJ) locations.get(i)).getY());
            }
