    ...
    public void propagateScale(int index) {
        ...
        if (index >= nImages || index < 0) {
            if (impl != null && ImageProcessorOJ.isFrontImageLinked()) {
                thisImg = images.getImageByName(impl.getTitle());
...
//            return ImageProcessorOJ.extractImageName(imp.getTitle());
    ...
    /**
     * used for synchronization: copy image properties into the ojj project
     *
     * @param imp
     * @param image
     */
    public void applyImageProperties(ImagePlus imp, ImageOJ image) {
        ...
        image.setVoxelSizeX(imp.getCalibration().pixelWidth);
        image.setVoxelSizeY(imp.getCalibration().pixelHeight);
        image.setVoxelSizeZ(imp.getCalibration().pixelDepth);
        //set frame settings
    ...
    public void updateOpenImages() {
        for (Enumeration e = openedImages.keys(); e.hasMoreElements();) {
            ...
            if (imp.getWindow() != null) {
                ...
                // TODO : gonna see if this actually works to fix ExtendVisibilityDepth
                ImageCanvas cnv = imp.getWindow().getCanvas();
                //if (cnv instanceof CustomCanvasOJ){
                //	cnv.paint()
                //}
