...
//            
            ...
            if (imp != null) {
            	/*
            	 * @ TODO
            	 * It looks like there's supposed to be stack fetching here.
            	 */
                if (imp.isHyperStack()) {
                ...
                /*
                 * @ TODO
                 * Check if centering x-y works
                 */
                
                // try to center image
                double origMag = imp.getCanvas().getMagnification();
                // cell should already be selected, so let's try using the getXPos and getYPos functions
                // ew this is disgusting
                
                //imp.getCanvas().setSourceRect()
                
                int aX = (int)OJ.getMacroProcessor().getXPos(1);
                int aY = (int)OJ.getMacroProcessor().getYPos(1);
                
                imp.getCanvas().zoom100Percent();
                
                imp.getCanvas().zoomIn(aX, aY);
                //imp.getCanvas().zoomOut(aX, aY);
     ...
     */
    /* 
     * @ NOTETOSELF : use this to change a given ytem to new one
     * Not quite predictable what this will do to non-point items
    */ 
    ...
    public LocationOJ setLocation(YtemOJ ytem, int locationIndex, LocationOJ location) {
        ...
        // TODO : to get the homeslice and the z position to play nice
        if (ytem.getStackIndex() != (int)loc.getZ())
        	ytem.setStackIndex((int)loc.getZ());
        qualifyCell(ytem.getCell());
        OJ.getEventProcessor().fireYtemChangedEvent();
        return loc;
    }
